...
def maxbytype(items) :
    ...
    for x in items:
        if type(x) is int:
            if largest_int is None or x> largest_int:
                largest_int=x
        if type(x)is float:
            if largest_float is None or x> largest_float:
                largest_float=x
        if type(x) is str:
            if largest_str is None or x>largest_str:
                largest_str=x
    return (largest_int, largest_float, largest_str)
